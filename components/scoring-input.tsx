...
export function ScoringInput({ playerName, currentScore, finishMode, onSubmitTurn }: ScoringInputProps) {
  ...
  const handleDartboardInput = useCallback((value: number, multiplier: 1 | 2 | 3) => {
    setDarts(prev => {
      ...
      // Find the current active dart index from state
      const currentActive = newDarts.findIndex(d => d.state === "empty")
      const idx = currentActive >= 0 ? currentActive : 2

      const state: DartState = value === 0 ? "miss" : "scored"
      newDarts[idx as 0 | 1 | 2] = { value, multiplier, state }
      return newDarts
    })

    // Advance to next empty dart
    setActiveDart(prev => {
      // Find next empty after filling current
      const nextEmpty = darts.findIndex((d, i) => i > prev && d.state === "empty")
      if (nextEmpty >= 0 && nextEmpty <= 2) return nextEmpty as 0 | 1 | 2
      // If no empty after, find any empty
      const anyEmpty = darts.findIndex((d, i) => i !== prev && d.state === "empty")
      if (anyEmpty >= 0 && anyEmpty <= 2) return anyEmpty as 0 | 1 | 2
      return 2 as 0 | 1 | 2
    })
  }, [darts])
  ...
  return (
    <Card className="bg-card border-border">
      ...
      <CardContent className="space-y-2 px-3 pb-3">
            ...
            return (
              <div key={index} className="relative">
                <button
                  ...
                  className={`relative w-full p-2 rounded-lg text-center transition-all select-none border-2 ${getDartStateStyle(dart, isActive)}`}
                >
                  <div className={`text-[10px] ${isActive ? "opacity-90" : "opacity-60"}`}>
                    {t.dart} {index + 1}
                  </div>
                  <div className="text-lg font-bold leading-tight">{formatDartDisplay(dart)}</div>
                  <div className={`text-xs font-medium ${isActive ? "opacity-90" : "opacity-60"}`}>
        ...
        {/* Total */}
        <div className="flex items-center justify-between px-2 py-1.5 bg-secondary/50 rounded">
            ...
            <div className="grid grid-cols-6 gap-1">
              {HIGH_VALUES.map((value) => (
                <Button
                  key={value}
                  variant="secondary"
                  className="h-9 text-sm font-medium bg-secondary text-secondary-foreground hover:bg-secondary/80 px-0"
            ...
            <div className="grid grid-cols-7 gap-1">
              {LOW_VALUES.map((value) => (
                <Button
                  key={value}
                  variant="secondary"
                  className="h-8 text-sm font-medium bg-secondary/70 text-secondary-foreground hover:bg-secondary/50 px-0"
                  onClick={() => handleValueClick(value)}
                >
                  {value}
                </Button>
        ...
        <div className="grid grid-cols-2 gap-2 pt-1">
          <Button
            variant="secondary"
            className="h-10 bg-secondary text-secondary-foreground text-sm"
            onClick={handleClear}
          >
